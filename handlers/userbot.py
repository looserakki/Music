...
from config import BOT_USERNAME, BOT_OWNER, z_version, kishuub_version
from callsmusic.callsmusic import client as KISHUUB
from handlers.ownerstuff import _check_heroku

# To Block a PM'ed User
@KISHUUB.on_message(filters.private & filters.command("block", [".", "/"]) & filters.me & ~filters.edited)
...
@KISHUUB.on_message(filters.command("unblock", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  good_bro = int(message.command[1])
  gonna_unblock_u = await message.edit_text("`Unblocking User...`")
  try:
    await KISHUUB.unblock_user(good_bro)
...
@KISHUUB.on_message(filters.private & filters.command("chats", [".", "/"]) & filters.me & ~filters.edited)
async def ubgetchats(_, message: Message):
  getting_chats = await message.edit_text("`Checking Your Chats, Hang On...`")
  async for dialog in KISHUUB.iter_dialogs():
    try:
      total = await KISHUUB.get_dialogs_count()
...
# Leave From a Chat
@KISHUUB.on_message(filters.command("kickme", [".", "/"]) & filters.me & ~filters.edited)
async def ubkickme(_, message: Message):
  i_go_away = await message.edit_text("`Leaving This Chat...`")
  try:
    await KISHUUB.leave_chat(message.chat.id)
...
# Alive Message
@KISHUUB.on_message(filters.command("alive", [".", "/"]) & filters.me & ~filters.edited)
async def ubalive(_, message: Message):
  ...
  await message.reply_photo(alive_pic, caption=f"**☑️ kishu Music Userbot is Alive 🌀** \n\n**🤖 Version** \n ↳**Bot Version:** `{z_version}` \n ↳**Userbot Version:** `{kishuub_version}` \n\n**🐬 Info**\n ↳**Music Bot:** @{BOT_USERNAME} \n ↳**Owner:** [Click Here](tg://user?id={BOT_OWNER})")
...
@KISHUUB.on_message(filters.private & filters.command("pmlogs", [".", "/"]) & filters.me & ~filters.edited)
@_check_heroku
async def getlogs(client: KISHUUB, message: Message, app_):
  ...
  if status == "on":
    ...
    try:
      ...
      pmchat = await KISHUUB.create_group(f"Userbot's PM Logs", BOT_OWNER)
      chat_id = pmchat.id
      await KISHUUB.set_chat_photo(chat_id=chat_id, photo=chat_pic)
      await logmsg.edit(f"`Successfully Finished Step 1, To Enable This Feature Please Check Your Log Group That Created Now!!` \n\n ~ **@SUPERIOR_BOTS**")
      await client.send_message(chat_id, f"**Welcome to @{(await KISHUUB.get_me()).username}'s PM Log Group!** \nThis Chat will Contain All PM Messages Of **@{(await KISHUUB.get_me()).username}** ! \n\n\n`/setvar PM_LOG_CHAT_ID {chat_id}` \n\n ✪ **Please Copy and Send Above Command To Your @{BOT_USERNAME} Now**!")
    except Exception as lol:
    ...
    try:
      ...
      await KISHUUB.leave_chat(PM_LOG_CHAT_ID, delete=True)
...
@KISHUUB.on_message(filters.private)
async def sendpmlol(client: ZAIDUB, message: Message):
  KISHUUB_ID = int((await KISHUUB.get_me()).id)
  if message.from_user.id == BOT_OWNER or message.from_user.id == KISHUUB_ID:
    return
  pmlogchat = PM_LOG_CHAT_ID
  userinfo = await KISHUUB.get_users(user_ids=message.from_user.id)
